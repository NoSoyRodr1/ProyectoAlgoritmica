...
public:
    ...
    void cargarUsuarios() {
        ...
        while (getline(file, line)) {
            ...
            string usr, pwd, monStr, promStr;
            getline(ss, usr, ',');
            getline(ss, pwd, ',');
            getline(ss, monStr, ',');
            getline(ss, promStr, ',');  // Leer el contador de promociones
            try {
                mon = stod(monStr); // Convertir string a double
                prom = stoi(promStr); // Convertir string a int
                usuarios.push_back({usr, pwd, mon, prom});
    ...
    void hacerReservaCochera(const string& username) {
        ...
        gotoxy(2, 8); cout << "Ingrese el ID de la cochera: ";
        cin >> cocheraId;
        ...
        gotoxy(2, 9); cout << "Fecha de inicio (DD/MM/YYYY): ";
        cin >> fechaInicio;
        ...
        gotoxy(2, 10); cout << "Fecha de fin (DD/MM/YYYY): ";
        cin >> fechaFin;
        if (!esFechaValida(fechaFin) || diasEntreFechas(fechaInicio, fechaFin)<0) {
    ...
    Reserva& obtenerUsuarioPorNombre(const string& username) {
    for (auto& usuario : reservas) {
    ...
    int obtenerContadorPromo(const string& username) {
            for (const auto& user : usuarios) {
                if (user.username == username) {
                    return user.countpromo;
                }
            }
            return 0;
        }
    ...
    void registrarUsuario(const string& username, const string& password) {
        ...
        file << username << "," << hashed_password << "," << 0 << "," << 0 << endl;
        file.close();
    ...
    void incrementarContadorPromo(const string& username, int contadorPromo) {
    for (auto& user : usuarios) {
        if (user.username == username) {
            user.countpromo = contadorPromo;
            ...
            for (const auto& u : usuarios) {
                file << u.username << "," << u.password_hash << "," << u.saldo << "," << u.countpromo << endl;
            }
            file.close();
            return;
        }
    ...
    void agregarSaldoUsuario() {
        system("cls");
		cuadro(0, 0, 80, 24);
        cuadro(1, 1, 77, 3);
        string usernam;
        double aumento;
        gotoxy(22,2); cout << "Sistema de ingreso de saldo";
        gotoxy(4,5);cout << "\n| Ingrese el usuario que desea añadir saldo:  ";
        cin >> usernam;
        gotoxy(4,7);double saldo = obtenerSaldoUsuario(usernam);
        cout << "\n| Ingrese el saldo que desea añadir:  ";
        cin >> aumento;
        saldo += aumento;
        actualizarSaldoUsuario(usernam, saldo);
        // Mostrar mensaje de recarga exitosa
        gotoxy(3,9);cout<< "\n\tRecarga de saldo realizada con exito.\n"
            << "\tUsuario: " << usernam << "\n"
            << "\tSaldo " << saldo
            << "\n\tPulse enter para continuar...";
        cin.ignore();
    ...
    double diasEntreFechas(const string& fechaInicio, const string& fechaFin) {
        ...
        // Calcular la diferencia de días - modif
        if(diasDesdeFin >= diasDesdeInicio) {
    ...
    void hacerReserva(const string& username) {
        double saldo = obtenerSaldoUsuario(username);
        int promocionContador = obtenerContadorPromo(username);
        ...
        gotoxy(2, 8); cout << "Ingrese el numero de la habitacion: ";
        cin >> habitacionId;
        ...
        // Verificacion de la promocion para el descuento
        if (promocionContador == 3) 
        {
           gotoxy(2, 11); cout << "¡FELICIDADES, USTED TENDRÁ UNA PROMOCIÓN ESPECIAL DE DESCUENTO DEL 50%!";
           montoPromocion = precioHabitacion*0.5;
           aplicoPromocion = true;
        }
        // Verificar si el usuario tiene saldo suficiente para reservar
        if (saldo < precioHabitacion*(diasEntreFechas(fechaInicio, fechaFin)+1) - montoPromocion) {
            cout << "Saldo insuficiente para hacer la reserva." << endl;
        ...
        if (it != habitaciones.end() ) {
            ...
            // Actualizar el saldo del usuario
            saldo = saldo - precioHabitacion*(diasEntreFechas(fechaInicio, fechaFin)+1) + montoPromocion;
            if (!aplicoPromocion && promocionContador!=4) 
            {
               promocionContador = promocionContador + 1;
            }
            else
            {
                promocionContador = 4;
            ...
            actualizarSaldoUsuario(username, saldo);
            incrementarContadorPromo(username, promocionContador);
            // Guardar cambios
            guardarHabitaciones();
            guardarReservas();
    ...
    void cancelarReserva(const string& username) {
        ...
        if (it != reservas.end()) {
            ...
            double montoReembolso = precioHabitacion;
            int Count=obtenerContadorPromo(username);
        if (Count>=3) {
            montoReembolso = precioHabitacion*0.5; // Reembolso con la promocion (habitacion respectiva con la promocion en proceso aun)
            incrementarContadorPromo(username, 0);
        }
        else
        {
            Count = Count - 1;
            incrementarContadorPromo(username, Count);
        }
            double saldoUsuario = obtenerSaldoUsuario(username);
            double nuevoSaldo = saldoUsuario + montoReembolso;
    ...
    void cancelarReservaAdm() {
        ...
        if (it != reservas.end()) {
            ...
            string fechaInicio = it->fechaInicio;
            string fechaFin = it->fechaFin;
            string userN = it->username;

            // Reembolsar el saldo al usuario
            double precioHabitacion = obtenerPrecioHabitacion(it->habitacionId) * (diasEntreFechas(fechaInicio, fechaFin) + 1);
            double montoReembolso = precioHabitacion;
            int Count=obtenerContadorPromo(userN);
        if (Count>=3) {
            montoReembolso = precioHabitacion*0.5; // Reembolso con la promocion (habitacion respectiva con la promocion en proceso aun)
            incrementarContadorPromo(userN, 0);
        }
        else
        {
            Count = Count - 1;
            incrementarContadorPromo(userN, Count);
        }
            double saldoUsuario = obtenerSaldoUsuario(userN);
            double nuevoSaldo = saldoUsuario + montoReembolso;
            actualizarSaldoUsuario(userN, nuevoSaldo);
    ...
    void mostrarHabitacionesDisponiblesPorTipo(const std::string& tipo) {
        ...
        for (const auto& hab : habitaciones) {
            if (hab.tipo == tipo && hab.disponible) {
                gotoxy(2, y++);
                std::cout << "Numero: " << hab.id << ", Precio: " << hab.precio << " (Con promoción: " << hab.precio * 0.5 << " )";
            }
        }
        std::cout << "\n\t\t\tPulse enter para continuar...";
        std::cin.ignore();
    ...
    void mostrarTiposDeHabitaciones() {
        ...
        cuadro(0, 0, 80, 24);
        cuadro(1, 1, 77, 3);
        gotoxy(25, 2); std::cout << "Seleccionar Tipo de Habitacion";
...
void menu_personal(int nivel) {
	...
	const char *opciones1[] = {"Ver reservas", "Ver habitaciones disponibles", "Ver cochera disponible","Ver personal activo","Eliminar personal", "Añadir saldo a usuario","Cancelar reserva habitaciónss" ,"Retroceder al menu principal"};
    const char *opciones2[] = {"Ver reservas", "Ver habitaciones disponibles", "Ver cochera disponible","Retroceder al menu principal"};
    int m1=8,m2=4;
    if(nivel==1){
        do{
        ...
        if (opt == 1) {
            ...
            um.agregarSaldoUsuario();
        } else if(opt == 7) {
            um.cancelarReservaAdm();
        }else if(opt == 8) {
            repite = false;
            break;
        }
